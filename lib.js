...
/* Sorteia a condição de campo/clima uma vez no início da partida. */
export function sortearClima() {
  const r = Math.random();
  let acc = 0;
  for (const c of CONDICOES_CLIMA) { acc += c.chance; if (r < acc) return c; }
  return CONDICOES_CLIMA[0];
}

/* Sorteia a formação tática pra clássicos/finais — decisão do técnico, não sua. */
/* Checa se dois clubes têm rivalidade de clássico de verdade (lista
   pesquisada), não uma aproximação por estado ou força parecida. */
export function ehClassicoReal(nomeA, nomeB) {
